...
function randomDomain() {
  let name = '';
  for (let i = 0; i < 6; i++) name += CONSONANTS[Math.floor(Math.random() * CONSONANTS.length)];
  return name;
}

function generateBatch(seen: Set<string>): string[] {
  const batch: string[] = [];
...
export default function GeneratorPage() {
  ...
  async function handleStart() {
    ...
    while (found.length < TARGET && !stopRef.current) {
      const batch = generateBatch(seenRef.current);
  ...
  function handleCopy() {
    navigator.clipboard.writeText(available.map(d => `${d}.com`).join('\n'));
    setCopied(true);
    ...
    <main className="max-w-2xl mx-auto px-4 py-12">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1 text-sm">Find 50 available 6-letter consonant-only .com domains.</p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-8">
        {!running ? (
          ...
          >
            {done ? 'Run again' : 'Find 50 available domains'}
          </button>
        ) : (
          <button
            onClick={handleStop}
          ...
          <div className="grid grid-cols-3 divide-x divide-y divide-gray-800/50">
            {available.map((d, i) => (
              ...
              >
                ...
                <span className="font-mono text-sm text-white">{d}.com</span>
              </div>
            ))}
          </div>
        </div>
